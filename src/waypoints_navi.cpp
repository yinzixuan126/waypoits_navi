 ...
 *@brief: 从start状态，开始，进入active状态，判断任务进行中，判断任务是否超时
 * 判断任务是否完成和是否到达最后一个点，然后再次进入start
 * /
/*
 * TODO
...
bool WaypointsGoalNode::init()
{
    ...
    //添加订阅clicked_point
    clicked_point_sub_=nh.subscribe("/clicked_point",1,&WaypointsGoalNode::clickedpointCB,this);
...
void WaypointsGoalNode::clickedpointCB(const geometry_msgs::PointStamped& clickedpoint)
{
    yocs_msgs::Waypoint wp_clicked_point;
    wp_clicked_point.header=clickedpoint.header;
    wp_clicked_point.pose.position=clickedpoint.point;
    wp_clicked_point.pose.orientation.w=1.0;
    wp_list_.waypoints.push_back(wp_clicked_point);
    ROS_INFO_STREAM("Received " << wp_list_.waypoints.size() << " way points");
...
void WaypointsGoalNode::navCtrlCB(const yocs_msgs::NavigationControl::ConstPtr& nav_ctrl)
{
    ...
    else if ((nav_ctrl->control == yocs_msgs::NavigationControl::START))
    {
        ...
        {
            //resetWaypoints();//删除清楚目标点的功能，如果采用trajectory list uncomment this line
            ...
            {
                state_ = START;
                //mode_  = GOAL;原始是goal模式
                mode_ = LOOP;//后面改为loop模式
                ROS_ERROR("lOOP IS STARTING");
            }
            else
            {
...
void WaypointsGoalNode::spin()
{
    ...
    while (ros::ok())
    {
        rate.sleep();
        ros::spinOnce();
        //ROS_INFO("navi is working");

        if (state_ == START)
        {
            //ROS_INFO("state is start");
            if (mode_ == LOOP)
            {
                if (waypoints_it_ == waypoints_.end())
                {
                    waypoints_it_ = waypoints_.begin();
                    ROS_ERROR("loop is working");
                }
            }
        ...
        else if (state_ == ACTIVE)
        {
                    ...
                    (goal_state == actionlib::SimpleClientGoalState::PREEMPTED))
            {
                ...
                if ((ros::Time::now() - mb_goal.target_pose.header.stamp).toSec() >= goal_timeout_)
                {
                    ...
                    //if (waypoints_it_ < (waypoints_.end() - 1))//original
                    if (waypoints_it_ < (waypoints_.end()))//changed by me
                    {
                        ROS_ERROR("Requesting next way point.");
                        waypoints_it_++;
                        state_ = START;
                    }
                    else
                    {
                        ROS_ERROR("No more way points to go to.");
                        state_ = COMPLETED;
                    }
                }
                ...
                //检查是否到达目标点附近
                //if (waypoints_it_ < (waypoints_.end() - 1)) //original verson
                if (waypoints_it_ < (waypoints_.end())) //changed by me

                {
                    ...
                    if (distance <= close_enough_)
                    {
                        ...
                        ROS_INFO("the sequence is %d", waypoints_it_-waypoints_.begin());
                        if(waypoints_it_==waypoints_.end())
                        {ROS_ERROR("JIU ZAI ZHE LI");}
                    }
                    else
                    {
            ...
            else // actionlib::SimpleClientGoalState::SUCCEEDED, REJECTED, ABORTED, LOST
            {
                if (goal_state == actionlib::SimpleClientGoalState::SUCCEEDED)
                {
                    ROS_INFO("Go to goal successfully completed: %.2f, %.2f, %.2f",
                             mb_goal.target_pose.pose.position.x, mb_goal.target_pose.pose.position.y,
                             tf::getYaw(mb_goal.target_pose.pose.orientation));
                    //if (waypoints_it_ < (waypoints_.end() - 1))//原始倒数第二个点
                    if (waypoints_it_ < (waypoints_.end()))//我改为倒数第一个点
                    {
                        ROS_INFO("Requesting next way point.");
                        waypoints_it_++;
                        state_ = START;
                    }
                    else
                    {
                        ROS_INFO("Reached final way point.");
                        state_ = COMPLETED;
                    }
                }
                else//如果没有成功到达某个点
        ...
        {
            //ROS_INFO("state is completed");
            // publish update
            publishStatusUpdate(yocs_msgs::NavigationControlStatus::COMPLETED);
            idle_status_update_sent_ = false;
            //state_ = IDLE;//初始完成所有任务后，状态改为idle
            state_=START;//我改成了完成所有任务后，状态回到开始
        }
        else // IDLE
        {
            //ROS_INFO("state is idle");
